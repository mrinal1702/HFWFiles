...
**Active gameweek (as of GW1):** `game_week_id = 1`, name `FIFA World Cup Group Stage GW1`, `Is_Active = true` in `Game_Weeks`.
...
| Forward | 0 | −2.5 | **+3 per team goal** (not per personal goal) |
...
**Command** — after USA vs Paraguay example, refresh GW1 with all four matches:
