-- Lock Quarter-Final squads (GW6) for ALL production WC auctions (5, 6, 7) + activate GW6 on leaderboards.
-- Safe to re-run snapshot inserts (ON CONFLICT DO NOTHING).
-- Does NOT reopen next-round bidding or reset paid releases.
--
-- Run after the QF hard deadline (auction-qf-open-bidding-5-6-7.sql window closes).
-- Snapshots current auction_teams — the live QF squads managers built during bidding.
--
-- Leaderboard UI: pre–Best XI view (flat squad by listed position; sum of all player scores).
-- Run Best XI / auction_leaderboard publish only after QF matches complete.

-- ─── 0) Ensure GW6 exists in Game_Weeks ───────────────────────────────────────
...
-- ─── 1) Lock QF squads from current auction_teams ─────────────────────────────
...
-- ─── 2) Point leaderboards at GW6 (This Gameweek tab) ─────────────────────────
...
-- ─── Verify ──────────────────────────────────────────────────────────────────
-- select auction_id, count(*) as gw6_rows
-- from gameweek_squads
-- where game_week_id = 6 and auction_id in (5, 6, 7)
-- group by auction_id
-- order by auction_id;
