/**
 * Open UCL auction 10 bidding (pre-GW1 squad building after Live Auction 1 import).
 *
 * Does NOT:
 *   - touch gameweek_squads (GW1 lock happens at hard deadline)
 *   - change Game_Weeks.Is_Active
 *   - reset sold lots or budgets
 *
 * Does:
 *   - reset paid_release_used (1 paid release per manager)
 *   - reopen unsold lots to uninitiated (if any)
 *   - set hard deadline Mon 8 Sep 2026 16:15 Europe/Dublin
 *   - clear initiation/raise deadlines
 *   - open transfer window (no admin approval)
 *   - set max_participants = 15
 *
 * Usage:
 *   node scripts/open-ucl-auction-10-bidding.mjs --dry-run
 *   node scripts/open-ucl-auction-10-bidding.mjs
 */
...
const AUCTION_ID = 10;
const HARD_DEADLINE_ISO = "2026-09-08T15:15:00.000Z"; // 16:15 Europe/Dublin (IST, UTC+1)
...
async function snapshot(supabase) {
  const { data: auction, error: aErr } = await supabase
    .from("Auctions")
    .select(
      "id,name,is_active,bidding_deadline_mode,initiation_deadline_at,raise_deadline_at,hard_deadline_at,transfer_window_open,transfers_require_admin_approval,max_participants,competition_id",
    )
    .eq("id", AUCTION_ID)
    .maybeSingle();
  if (aErr) throw new Error(aErr.message);
  if (!auction) throw new Error("Auction 10 not found");
...
async function main() {
  ...
  console.log("Initiation / raise: null (hard deadline only, 24h rolling per lot)");
  console.log("Budget boost: none");
  console.log("Transfers: open, no admin approval");
  console.log(`max_participants: ${MAX_PARTICIPANTS}`);

  if (before.users.length !== MAX_PARTICIPANTS) {
    console.warn(
      `\n⚠️  Expected ${MAX_PARTICIPANTS} managers, found ${before.users.length} — continuing anyway.`,
    );
  }
  ...
  console.log(`✅ Reopened ${unsoldCount} unsold lots to uninitiated (if any)`);
  ...
  console.log("✅ Deadlines, transfer window, and max_participants updated");
